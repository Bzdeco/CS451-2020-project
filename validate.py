...
def main(processes, messages, runscript, broadcastType, logsDir, testConfig):
    ...
    if broadcastType == "fifo":
        ...
        dependencies = None  # randomly generated if None
        validation = LCausalBroadcastValidation(processes, messages, logsDir, dependencies)
...
def validateDirectory(processes, messages, directory):
    dependencies = {
        1: {2, 3},
        2: {3},
        3: set({})
    }
    validation = LCausalBroadcastValidation(processes, messages, directory, dependencies)
    validation.parseBroadcastDependencies()
    print('Validation result:', validation.checkAll())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        dest="directory",
        help="Directory with log files to validate."
    )
    ...
    if results.directory is not None:
        validateDirectory(results.processes, results.messages, results.directory)
    else:
